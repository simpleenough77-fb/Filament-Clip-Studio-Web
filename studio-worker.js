...
const python=loadPyodide({indexURL:new URL('./vendor/pyodide/',import.meta.url).href}).then(async py=>{
 ...
 const assets=await(await fetch('./assets.json?v=tested-supports-4')).json();
 py.FS.mkdir('/author');py.FS.mkdir('/Generated');py.FS.mkdir('/.cache');
 for(const name of assets){const path='/'+name;py.FS.mkdirTree(path.slice(0,path.lastIndexOf('/')));py.FS.writeFile(path,new Uint8Array(await(await fetch('./'+name+'?v=tested-supports-4')).arrayBuffer()));}
 await py.runPythonAsync('import sys\nsys.path.insert(0,"/author")\nimport generator as g\nimport json,base64\n');
 return py;
});
self.compileScad=(code,ext)=>new Promise((resolve,reject)=>{
 const w=new Worker('./compile-worker.js?v=tested-supports-4',{type:'module'});
