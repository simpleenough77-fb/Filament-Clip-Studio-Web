  ...
  for style in ['part','cut','modifier']:
   ...
   with zipfile.ZipFile(model) as z:
    ...
    for ob in objects:
     ...
     assert min(v[2] for v in vs)>=-1e-5 and max(v[2] for v in vs)<=33.00001
     if ob.get('name','').startswith('Clip body'):
      b=[(min(v[i] for v in vs),max(v[i] for v in vs)) for i in range(3)];assert abs(b[2][1]-b[2][0]-33)<1e-5;bounds.append(b)
